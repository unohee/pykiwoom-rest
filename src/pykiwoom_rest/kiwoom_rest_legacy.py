...
class KiwoomRestBase:
    ...
    def _convert_stock_code_param(
        ...
        """
        if legacy_format:
            return {"FID_COND_MRKT_DIV_CODE": "J", "FID_INPUT_ISCD": stock_code}
        else:
    ...
    def _get_access_token(self) -> str:
        ...
        try:
            ...
            raise requests.exceptions.JSONDecodeError(
                f"토큰 발급 실패: HTTP {response.status_code}, 응답이 JSON이 아님: {response.text[:200]}",
                e.doc,
                e.pos,
            )
        ...
        if response.status_code == 200 and result.get("return_code") == 0:
            ...
            error_detail = {
                ...
                "url": url,
                "request_data": {
                    k: v for k, v in data.items() if k != "secretkey"
                },  # 보안상 secret 제외
            }
    ...
    def _is_valid_response(self, response: dict) -> bool:
        ...
        return (
            response.get("return_code") == 0
            or response.get("rt_cd") == "0"
            or "stk_cd" in response  # 차트 데이터는 stk_cd 포함
            or len(response) > 0
        )  # 응답이 있으면 일단 유효
...
class KiwoomRest(KiwoomRestBase):
    ...
    def get_daily_chart(
        ...
        params = {"stk_cd": stock_code, "base_dt": end_date, "upd_stkpc_tp": "1"}
    ...
    def get_program_trading_daily(
        ...
        return self.make_request(
            "market_condition", "program_trading_by_stock_daily", params=params
        )
    ...
    def _fetch_minute_chart_page(
        self, stock_code: str, interval: int, start_date: str, end_date: str
    ) -> dict:
        """분봉 차트 페이지 조회"""
    ...
    def get_all_sector_index(self) -> dict:
        """전업종지수요청 (ka20003)"""
        params = {"FID_COND_MRKT_DIV_CODE": "U", "FID_INPUT_ISCD": "0000"}
    ...
    def get_theme_stocks(self, theme_code: str) -> dict:
        """테마구성종목요청 (ka90002)"""
        params = {"FID_COND_MRKT_DIV_CODE": "J", "FID_INPUT_ISCD": theme_code}
    ...
    def get_investor_daily_trading(
        self, stock_code: str, start_date: str = None, end_date: str = None
    ) -> dict:
    ...
    def get_stock_investor_institutional(
        self, stock_code: str, start_date: str = None, end_date: str = None
    ) -> dict:
        """종목별투자자기관별요청 (ka10059)"""
        if not end_date:
    ...
    def get_stock_investor_institutional_summary(
        self, stock_code: str, start_date: str = None, end_date: str = None
    ) -> dict:
    ...
    def get_program_trading_cumulative_trend(
        ...
        return self.make_request(
            "market_condition", "program_trading_cumulative_trend", params=params
        )

    def get_stock_time_program_trading_trend(
        self, stock_code: str, start_date: str = None, end_date: str = None
    ) -> dict:
        ...
        return self.make_request(
            "market_condition", "stock_time_program_trading_trend", params=params
        )
    ...
    def get_stock_daily_program_trading_trend(
        ...
        return self.make_request(
            "market_condition", "stock_daily_program_trading_trend", params=params
        )

    # ========== 차트 관련 추가 메서드 ==========

    def get_stock_investor_institutional_chart(
        self, stock_code: str, start_date: str = None, end_date: str = None
    ) -> dict:
    ...
    def get_sector_daily_chart(
        self, sector_code: str = "0001", start_date: str = None, end_date: str = None
    ) -> dict:
        """업종일봉조회요청 (ka20006)"""
        if not end_date:
            end_date = datetime.now().strftime("%Y%m%d")
    ...
    def get_sector_program(self, sector_code: str = "0001") -> dict:
        """업종프로그램요청 (ka10010)"""
        params = {"FID_COND_MRKT_DIV_CODE": "J", "FID_INPUT_ISCD": sector_code}
    ...
    def get_sector_current_price_daily(
        self, sector_code: str = "0001", start_date: str = None, end_date: str = None
    ) -> dict:
        """업종현재가일별요청 (ka20009)"""
        if not end_date:
    ...
    def get_lending_trading_trend_by_stock(
        self, stock_code: str, start_date: str = None, end_date: str = None
    ) -> dict:
