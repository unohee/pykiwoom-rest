...
class ConcurrentKiwoomAPI:
    ...
    def _fetch_async_batch(self, stock_codes: List[str]) -> BatchResult:
        ...
        try:
            return loop.run_until_complete(self._async_fetch_batch(stock_codes))
        finally:
            loop.close()
    ...
    def process_with_pipeline(self, data: List[Any], stages: List[Callable]) -> List[Any]:
        ...
        for stage in stages:
            if self.mode == ProcessingMode.THREAD_POOL:
                # 각 단계를 병렬 처리
                futures = [self._executor.submit(stage, item) for item in result]
...
class OptimizedBatchProcessor:
    ...
    def auto_process(self, stock_codes: List[str]) -> BatchResult:
        """자동 최적화 처리
